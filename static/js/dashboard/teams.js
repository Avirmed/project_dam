...
let tableTeamStationsID = "#tableTeamStations";
let tableTeamUsersID = "#tableTeamUsers";


function activeUserForm($form, jsonData) {
    $form.find(".nav.nav-tabs > .nav-item > .nav-link ").removeClass("disabled");

    let table = $form.find(".app-tabulator-table-tmp");
    table.removeClass("app-tabulator-table-tmp").addClass("app-tabulator-table");
    table.attr("cfg-extra-params", JSON.stringify({
        "viewType": "checkbox",
        // "filters": {"UserType": [1, 2, 3]},
...
function getCheckStationsAndUsers($form) {
    ...
    if (tableTeamStations === undefined || tableTeamUsers === undefined) return {};
    ...
    tableTeamUsers.getData().forEach(function (row) {
        if (row.checkbox === true) {
            jsonFields["Users"].push(row.UserID);
        }
    });

    $.each(jsonFields, function (key, value) {
        ...
        onShown: function () {
            ...
            if (cid != '') {
                $.get(`/api/${module}/get/${cid}`, function (jsonData) {
                    ...
                    activeUserForm(moduleForm, jsonData);
                }).fail(function (jqXHR) {
            ...
            moduleForm.ajaxForm({
                ...
                beforeSerialize: function ($form, options) {
                    ...
                    let jsonFields = getCheckStationsAndUsers($form);
                ...
                success: function (jsonData) {
                    if (jsonData.Data.TeamID == null || jsonData.Data.TeamID == '') {
                        table.setData();
                        dialog.modal("hide");
                        return;
                    }
            ...
            $(moduleForm).on("click", ".form-image .rotateBtn:not(.disabled) > span", function (e) {
                ...
                $.post(`/api/${module}/imgrotate`, { TeamID: $(`[name='TeamID']`).val(), direction: $(this).data("direction") }, function (jsonData) {
                    if (jsonData.Result) {
...
$(document).on("click", "#teamTable .addBtn", function (e) {
    loadForm();
});

$(document).on("click", "#teamTable .editBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    loadForm(rowId);
});

$(document).on("click", "#teamTable .deleteBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    ...
    bootbox.dialog({
        ...
        buttons: {
            yes: {
                ...
                callback: function () {
                    $.post(`/api/${module}/delete`, { TeamID: rowId }, function (jsonData) {
                        if (jsonData.Result) {
                            table.setData();
