...
function loadForm(cid = '') {
    ...
    let dialog = bootbox.dialog({
        ...
        onShown: function () {
            ...
            select2Ajax($("#FileTransferID"), "ID", "Hostname");
...
$(document).on("click", "#csvloggerTable .addBtn", function (e) {
    loadForm();
});

$(document).on("click", "#csvloggerTable .editBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    loadForm(rowId);
});

$(document).on("click", "#csvloggerTable .deleteBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    ...
    bootbox.dialog({
        ...
        message: `<h5>"${table.getRow(rowId).getData()['FilenameFormat']}"</h5>`,
        centerVertical: true,
        onEscape: true,
        size: 'md',
        buttons: {
