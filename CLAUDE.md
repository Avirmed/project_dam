...
- `event_watcher` — Security cameras drop `<IP>_<ch>_<yyyymmddHHMMSS>_<EVENT>.jpg` into `tmp/<EVENT_WATCH_FOLDER>`; files become `EventLog` rows (camera matched by RSTP/Onvif IP), images move to `static/data/events/<yyyymm>/`, unparseable files to `_unmatched/`. Public page `/events` (templates/modules/events, static/js/modules/events.js) with approve/reject.
- `image_uploader` — files under `tmp/<IMAGE_OUT_FOLDER>/<CameraID>/` are sent through the camera's Upload JPG (FTP) settings; delivered files move to `sent/`; outcome in `Camera.LastUploadRun/LastUploadResult`.

Related modules/endpoints: `POST /api/inbound/<DeviceID>` (device-facing REST API server, gated by `Station.Meta["API"]`: status, source allow-list, Basic/Bearer/API-Key auth, Inbound Data Mapping → `StationData` with `Data` mapped + `Raw`), `/api/stationdata` (read-only), `/api/httplog` (list/counters/get), `/api/eventlog` (list/counters/get/action). Public map status (`/api/stations/sites`) comes from the newest `StationData` per station vs the StationConfigures thresholds; older than `DATA_TIMEOUT_MINUTES` → "No connection". TLS certificate uploads for the REST API tab live in `certs/<StationID>/` (git-ignored, never web-served). Settings rows seeded by fixtures: `DATA_TIMEOUT_MINUTES`, `WORKER_ENABLED`, `EVENT_WATCH_FOLDER`, `IMAGE_OUT_FOLDER`.
