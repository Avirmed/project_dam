...
$(document).on("click", "#filetransferTable .addBtn", function (e) {
    loadForm();
});

$(document).on("click", "#filetransferTable .editBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    loadForm(rowId);
});

$(document).on("click", "#filetransferTable .deleteBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    ...
    bootbox.dialog({
        ...
        message: `<h5>"${table.getRow(rowId).getData()['Hostname']}"</h5>`,
        centerVertical: true,
        onEscape: true,
        size: 'md',
        buttons: {
