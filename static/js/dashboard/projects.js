        ...
        onShown: function () {
            ...
            moduleForm.ajaxForm({
                ...
                success: function (jsonData) {
                    if (jsonData.Data.ProjectID == null || jsonData.Data.ProjectID == '') {
                        table.setData();
                        dialog.modal("hide");
                        return;
                    }
                    ...
                    if (fileUploader.Files.length > 0) {
                        fileUploader.ContentID = jsonData.Data.ProjectID;
                        fileUploader.upload();
                        return;
                    }
            ...
            $(moduleForm).on("click", ".form-image .rotateBtn:not(.disabled) > span", function (e) {
                ...
                $.post(`/api/${module}/imgrotate`, { ProjectID: $(`[name='ProjectID']`).val(), direction: $(this).data("direction") }, function (jsonData) {
                    if (jsonData.Result) {
...
$(document).on("click", "#projectTable .addBtn", function (e) {
    loadForm();
});

$(document).on("click", "#projectTable .editBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    loadForm(rowId);
});

$(document).on("click", "#projectTable .deleteBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    ...
    bootbox.dialog({
        ...
        message: `<h5>"${table.getRow(rowId).getData()['ProjectName']}"</h5>`,
        centerVertical: true,
        onEscape: true,
        size: 'md',
        buttons: {
            yes: {
                ...
                callback: function () {
                    $.post(`/api/${module}/delete`, { ProjectID: rowId }, function (jsonData) {
                        if (jsonData.Result) {
                            table.setData();
