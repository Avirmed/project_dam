        ...
        onShown: function () {
            ...
            moduleForm.ajaxForm({
                ...
                success: function (jsonData) {
                    if (jsonData.Data.RiverBasinID == null || jsonData.Data.RiverBasinID == '') {
                        table.setData();
                        dialog.modal("hide");
                        return;
                    }
                    ...
                    if (fileUploader.Files.length > 0) {
                        fileUploader.ContentID = jsonData.Data.RiverBasinID;
                        fileUploader.upload();
                        return;
                    }
            ...
            $(moduleForm).on("click", ".form-image .rotateBtn:not(.disabled) > span", function (e) {
                ...
                $.post(`/api/${module}/imgrotate`, { RiverBasinID: $(`[name='RiverBasinID']`).val(), direction: $(this).data("direction") }, function (jsonData) {
                    if (jsonData.Result) {
...
$(document).on("click", "#riverbasinTable .addBtn", function (e) {
    loadForm();
});

$(document).on("click", "#riverbasinTable .editBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    loadForm(rowId);
});

$(document).on("click", "#riverbasinTable .deleteBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    ...
    bootbox.dialog({
        ...
        message: `<h5>"${table.getRow(rowId).getData()['WatershedName']}"</h5>`,
        centerVertical: true,
        onEscape: true,
        size: 'md',
        buttons: {
            yes: {
                ...
                callback: function () {
                    $.post(`/api/${module}/delete`, { RiverBasinID: rowId }, function (jsonData) {
                        if (jsonData.Result) {
                            table.setData();
