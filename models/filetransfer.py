...
class FileTransfer(db.Model):
    __tablename__ = "tbl_filetransfer"

    sort = [
        {"column": "Hostname", "field": "Hostname", "dir": "asc"},
    ]

    searchFields = ["Hostname"]

    required_fields = ["StationID", "Hostname"]
    ...
    def __repr__(self):
        return f"<FileTransfer {self.ID}:{self.Hostname}>"
    ...
    def fix_sequence(cls):
        try:
            ...
            with db.engine.begin() as conn:
                conn.execute(text("""
                        SELECT setval(
                            pg_get_serial_sequence('tbl_filetransfer', 'ID'),
                            (SELECT MAX("ID") FROM "tbl_filetransfer")
                        )
                    """))
