...
$(document).on("click", "#samplingTable .addBtn", function (e) {
    loadForm();
});

$(document).on("click", "#samplingTable .editBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    loadForm(rowId);
});

$(document).on("click", "#samplingTable .deleteBtn", function (e) {
    let rowId = $(this).closest(".tabulator-row").data('id');
    ...
    bootbox.dialog({
        ...
        message: `<h5>"${table.getRow(rowId).getData()['SamplingID']} - ${table.getRow(rowId).getData()['SamplingName']}"</h5>`,
        centerVertical: true,
        onEscape: true,
        size: 'md',
        buttons: {
