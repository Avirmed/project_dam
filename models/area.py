...
class Area(db.Model):
    ...
    sort = [
        {"column": "AreaID", "field": "AreaID", "dir": "asc"},
    ]

    searchFields = ["AreaID"]

    required_fields = ["AreaID", "AreaRef", "AreaDate"]

    ID = db.Column(db.Integer, primary_key=True)

    AreaID = db.Column(db.String(100), nullable=False)
    ...
    def __repr__(self):
        return f"<Area {self.ID}:{self.AreaID}>"
    ...
    def fix_sequence(cls):
        try:
            ...
            with db.engine.begin() as conn:
                conn.execute(text("""
                        SELECT setval(
                            pg_get_serial_sequence('tbl_area', 'ID'),
                            (SELECT MAX("ID") FROM "tbl_area")
                        )
                    """))
