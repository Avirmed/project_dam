...
class Sampling(db.Model):
    __tablename__ = "tbl_sampling"

    sort = [
        {"column": "SamplingID", "field": "SamplingID", "dir": "asc"},
        {"column": "SamplingName", "field": "SamplingName", "dir": "asc"},
    ]

    searchFields = ["SamplingID", "SamplingName"]

    required_fields = ["SamplingID", "SamplingName", "SamplingDate"]

    ID = db.Column(db.Integer, primary_key=True)

    SamplingID = db.Column(db.String(100), nullable=False)
    ...
    def __repr__(self):
        return f"<Sampling {self.ID}:{self.SamplingID}-{self.SamplingName}>"
    ...
    def fix_sequence(cls):
        try:
            ...
            with db.engine.begin() as conn:
                conn.execute(text("""
                        SELECT setval(
                            pg_get_serial_sequence('tbl_sampling', 'ID'),
                            (SELECT MAX("ID") FROM "tbl_sampling")
                        )
                    """))
