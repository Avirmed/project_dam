...
| `export_onnx.py` | Exports the uploaded `.pt` models to `.onnx` (opset 12, static input); run automatically after a model upload, by `app_run.bat` at start-up (`--missing`, sizes from the DB) and by the worker when the onnx backend needs one; or by hand on a PC where torch works. |
