...
class Project(db.Model):
    ...
    drfFilePath = os.path.join(statictext.APP_STATIC_PATH, "data", "project")
    filePath = os.path.join("/static", "data", "project")
    img_size = {
    ...
    searchFields = ["ProjectName", "CustomerName"]

    required_fields = ["ProjectName", "CustomerName"]

    ProjectID = db.Column(db.Integer, primary_key=True)

    ProjectName = db.Column(db.String(500), nullable=False)
    CustomerName = db.Column(db.String(500), nullable=False)
    ...
    project_station = db.relationship("Station", back_populates="project")
    ...
    @classmethod
    def getData(cls, id):
        object = cls.query.filter(cls.ProjectID == id).first()
    ...
    def fix_sequence(cls):
        try:
            with db.engine.connect() as conn:
                conn.execute(text("""
                        SELECT setval(
                            pg_get_serial_sequence('tbl_project', 'ProjectID'),
