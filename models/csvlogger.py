...
class CsvLogger(db.Model):
    __tablename__ = "tbl_csvlogger"

    sort = [
        {"column": "FilenameFormat", "field": "FilenameFormat", "dir": "asc"},
    ]

    searchFields = ["FilenameFormat"]

    required_fields = ["FileTransferID", "FilenameFormat"]
    ...
    FilenameFormat = db.Column(db.String(250), nullable=False)
    ...
    filetransfer = db.relationship("FileTransfer")

    def __repr__(self):
        return f"<CsvLogger {self.ID}:{self.FilenameFormat}>"
    ...
    def serialize(self):
        ...
        data["FileTransferHostname"] = (
            self.filetransfer.Hostname if self.filetransfer else None
        )
    ...
    def fix_sequence(cls):
        try:
            ...
            with db.engine.begin() as conn:
                conn.execute(text("""
                        SELECT setval(
                            pg_get_serial_sequence('tbl_csvlogger', 'ID'),
                            (SELECT MAX("ID") FROM "tbl_csvlogger")
                        )
                    """))
