...
class RiverBasin(db.Model):
    __tablename__ = "tbl_riverbasin"

    drfFilePath = os.path.join(statictext.APP_STATIC_PATH, "data", "riverbasin")
    filePath = os.path.join("/static", "data", "riverbasin")
    img_size = {
    ...
    sort = [
        {"column": "SortOrder", "field": "SortOrder", "dir": "asc"},
    ]

    searchFields = ["WatershedName"]

    required_fields = ["WatershedName"]

    RiverBasinID = db.Column(db.Integer, primary_key=True)

    WatershedName = db.Column(db.String(500), nullable=False)

    SortOrder = db.Column(db.SmallInteger, default=1, index=True)
    ...
    riverbasin_station = db.relationship("Station", back_populates="riverbasin")

    def __repr__(self):
        return f"<RiverBasin {self.RiverBasinID}:{self.WatershedName}>"
    ...
    @classmethod
    def getData(cls, id):
        object = cls.query.filter(cls.RiverBasinID == id).first()
    ...
    def fix_sequence(cls):
        try:
            with db.engine.connect() as conn:
                conn.execute(text("""
                        SELECT setval(
                            pg_get_serial_sequence('tbl_riverbasin', 'RiverBasinID'),
                            (SELECT MAX("RiverBasinID") FROM "tbl_riverbasin")
                        )
                    """))
