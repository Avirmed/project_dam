...
- `csv_logger` — every `LogInterval` minutes (clock-aligned) appends a row to `data/csv/<logger>/<FilenameFormat>` and uploads it via the logger's File Transfer (`services/file_transfer.py`, stdlib `ftplib`, FTP/FTPS explicit+implicit, directory structure); outcome in `CsvLogger.LastRun/LastResult`.
- `event_watcher` — Security cameras drop `<IP>_<ch>_<yyyymmddHHMMSS>_<EVENT>.jpg` into `Camera.eventWatchPath` (`data/security_in`); files become `EventLog` rows (camera matched by RSTP/Onvif IP), images move to `EventLog.drfFilePath` (`static/data/events/<yyyymm>/`, served from `EventLog.filePath`), unparseable files to `_unmatched/`. Public page `/events` (templates/modules/events, static/js/modules/events.js) with approve/reject.
- `image_uploader` — files under `Camera.imageOutPath/<CameraID>/` (`data/images_out`) are sent through the camera's Upload JPG (FTP) settings; delivered files move to `sent/`; outcome in `Camera.LastUploadRun/LastUploadResult`. Folder paths are class attributes (like `Station.drfFilePath`), not Settings rows.

**Runtime folders** (all git-ignored): `tmp/` is only for chunked front-end uploads (`*.partN` + assembled file; `/main/cleartmp` removes leftovers older than 24 h). `data/` (`statictext.APP_DATA_PATH`) holds private worker files that must never be web-served: `csv/`, `security_in/`, `images_out/`. Anything the public front must show (approved event images, future CCTV snapshots) goes under `static/data/`.
- `snapshot` — every `SNAPSHOT_INTERVAL_MINUTES` (Settings, 0 = off) downloads each enabled camera's ISAPI picture (`Camera.build_links()["SnapshotURL"]`, Basic/Digest auth) to `Camera.snapshotPath/<CameraID>/latest.jpg` (`static/data/cameras/`, public, for the front CCTV page). Taken-at time = file mtime, exposed as `SnapshotImage`/`SnapshotTime` in `Camera.serialize()` and previewed on the camera form. At most `MAX_PER_TICK` cameras per tick and one attempt per camera per interval (unreachable cameras cost `TIMEOUT_SECONDS` each and never block the other jobs for long).
...
- `simulator` — demo only (`SIMULATOR_ENABLED` setting, fixtures default 0): every 15-minute slot writes one realistic payload per active station straight into `tbl_station_data` (levels from the station's own thresholds, a few stations pinned to Warning / Critical / silent, `StationData.apply_flow` for FLOW); `services/simulator.py::backfill(days)` seeds history. Never enable on a real installation.
