...
class Team(db.Model):
    ...
    drfFilePath = os.path.join(statictext.APP_STATIC_PATH, "data", "team")
    filePath = os.path.join("/static", "data", "team")
    img_size = {
    ...
    sort = [
        {"column": "TeamName", "field": "TeamName", "dir": "asc"},
    ]
    ...
    @classmethod
    def getData(cls, id):
        object = cls.query.filter(cls.TeamID == id).first()
    ...
    def fix_sequence(cls):
        try:
            ...
            with db.engine.begin() as conn:
                conn.execute(text("""
                        SELECT setval(
                            pg_get_serial_sequence('tbl_team', 'TeamID'),
                            (SELECT MAX("TeamID") FROM "tbl_team")
                        )
                    """))
